...
class Combatant():
    ...
    def cbt_roll(self, mode = 'a'):
        ...
        return random.randint(1,10) + random.randint(1,10) + int(self.skill) + modifier
...
def combat(peep, groups):
    ...
    result = "{} attacks {} with a {}, who defends with a {}".format(peep.name, opponent.name, atk, defense)
    if damage > 0:
        opponent.take_hit(damage)
        result += ". {} takes {} damage".format(opponent.name, damage)
        result += "\n{} is at {} hit points".format(opponent.name, opponent.current_hp)
